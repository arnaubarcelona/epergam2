...
<br>
    ...
    <?php foreach ($pdocuments as $documents): ?>
		<?php $nummm = $nummm + 1 ?>
			<table>
			  <tr><td rowspan="3" width="100px"><center><b><?= $nummm . ' / ' . $total ?></b></td><td width="120px" rowspan="3">
			  <?php if (!empty($documents->photo_dir)): ?>
			  <img class="portada" style="max-width: 100px; max-height: 100px" src="http://80.211.14.98/epergam2/<?php echo $documents->photo_dir?>/thumbnail-<?php echo $documents->photo?>" alt="<?php echo $documents->name ?>" />
			  <?php else: ?>
			  <?php if(!empty($documents->format)):?>
			  <img style="max-width: 100px; max-height: 100px" src="http://80.211.14.98/epergam2/<?php echo $documents->format->photo_dir?>/<?php echo $documents->format->photo?>" alt="<?php echo $documents->format->name ?>" />
			  <?php endif;?> 
			  <?php endif;?> 
			  </td></tr><tr>      
