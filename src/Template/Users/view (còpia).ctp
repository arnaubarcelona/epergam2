...
<div class="noudiv">
      <h3><?= h($user->name) ?> <a href="http://80.211.14.98/epergam2/users/pdfviewall/<?php echo $user->id?>.pdf"><img width="24px" height="24px" src="http://80.211.14.98/epergam2/webroot/img/icons/printer.png"></a></h3>
      Prestar: <?php echo $this->Form->create(null);
        ...
        <tr>
		    <td><center><a href="http://80.211.14.98/epergam2/users/edit/<?= $user->id ?>"><?= $user->id ?></a></td>            
    ...
    <table class="responsive-table">
		  <tr>
			<td rowspan="2" colspan="1" style="vertical-align: middle; text-align:center; cursor:pointer" onclick="window.location='http://80.211.14.98/epergam2/documents/view/<?php echo $documents->id ?>'">			  
			  <center><b><?= $nummm . ' / ' . $total ?></b></center><br>
			  <?php if (!empty($documents->photo_dir)): ?>
			  <img class="portada" style="max-width: 150px; max-height:250px;" src="http://80.211.14.98/epergam2/<?php echo $documents->photo_dir?>/thumbnail-<?php echo $documents->photo?>" alt="<?php echo $documents->name ?>" /></a>
			  <?php else: ?>
		  ...
		  <td colspan=4 style="font-size: 1.25em; font-weight: bold; text-align:left; cursor:pointer" onclick="window.location='http://80.211.14.98/epergam2/documents/view/<?php echo $documents->id ?>'">
