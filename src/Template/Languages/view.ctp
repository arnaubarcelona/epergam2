...
<div class="noudiv">
    <h3><?php $this->loadHelper('Custom');?><?= $this->Custom->viewLanguage($language->photo_dir, $language->photo, null) ?> <?= __('Documents') ?> en <?= h($language->name) ?></h3>
        ...
        <tr>
            ...
            <td><center><?= h($language->created->i18nFormat('d/M/YYYY hh:mm')) ?></td>            
            <td><center><?= h($language->modified->i18nFormat('d/M/YYYY hh:mm')) ?></td>
        </tr>
    </table>
	<div class="paginator">
		<table class="responsive-table">
