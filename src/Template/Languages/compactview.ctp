...
<div class="noudiv">
    <h3><?php $this->loadHelper('Custom');?><?= $this->Custom->viewLanguage($language->photo_dir, $language->photo, null) ?> <?= __('Documents') ?> en <?= h($language->name) ?></h3>
        ...
        <tr>
            ...
            <td><?= h($language->created->i18nFormat('d/M/YYYY hh:mm')) ?></td>            
            <td><?= h($language->modified->i18nFormat('d/M/YYYY hh:mm')) ?></td>
        </tr>
    </table>	
<div class="paginator">
		<table class="responsive-table">
		...
		<a href="http://80.211.14.98/epergam2/languages/pdfcompactview/<?php echo $language->id?>.pdf"><img width="24" height="24" src="http://80.211.14.98/epergam2/webroot/img/icons/printer.png"> </b></a><a href="http://80.211.14.98/epergam2/languages/view/<?php echo $language->id?>"><b>Llista detallada</b></a> | <b>Ordena per:</b> <?= $this->Paginator->sort('name', 'Títol') ?> | <?= $this->Paginator->sort('format_id', 'Format') ?> | <?= $this->Paginator->sort('cdu_id', 'CDU') ?> | <?= $this->Paginator->sort('location_id', 'Ubicació') ?> | <?= $this->Paginator->sort('lending_state_id', 'Préstec') ?> | <?= $this->Paginator->sort('id') ?> | <?= $this->Paginator->sort('created', 'Data de registre') ?>
		</td>
...
<table class="responsive-table">
    ...
    <?php $nummm = $nummm + 1 ?>    
		  <tr>  
			  ...
			  <b><a href="http://80.211.14.98/epergam2/documents/view/<?php echo $documents->id ?>"><?= $nummm ?> / <?php echo count($language->documents)?></a></b> &emsp;&emsp;
			  <?php elseif(strlen($nummm)==2):?>
			  <b><a href="http://80.211.14.98/epergam2/documents/view/<?php echo $documents->id ?>"><?= $nummm ?> / <?php echo count($language->documents)?></a></b>&emsp;&emsp;
			  <?php elseif(strlen($nummm)==3):?>
			  <b><a href="http://80.211.14.98/epergam2/documents/view/<?php echo $documents->id ?>"><?= $nummm ?> / <?php echo count($language->documents)?></a></b> &emsp;
	...
	<div class="paginator">
		...
		<tr>
		<td colspan="2"  style="height: 42px">
		<a href="http://80.211.14.98/epergam2/languages/compactview/<?php echo $language->id?>.pdf"><img width="24" height="24" src="http://80.211.14.98/epergam2/webroot/img/icons/printer.png"> </b></a><a href="http://80.211.14.98/epergam2/languages/view/<?php echo $language->id?>"><b>Llista detallada</b></a> | <b>Ordena per:</b> <?= $this->Paginator->sort('name', 'Títol') ?> | <?= $this->Paginator->sort('format_id', 'Format') ?> | <?= $this->Paginator->sort('cdu_id', 'CDU') ?> | <?= $this->Paginator->sort('location_id', 'Ubicació') ?> | <?= $this->Paginator->sort('lending_state_id', 'Préstec') ?> | <?= $this->Paginator->sort('id') ?> | <?= $this->Paginator->sort('created', 'Data de registre') ?>
		</td>
		<?php $numpages = $this->Paginator->counter(['format' => '{{pages}}']) ?>
		<td colspan="2">
		<?php if($numpages == "1"):?>
